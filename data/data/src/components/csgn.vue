  ...
  <div class="container">
        ...
        <div class="right nav text_right">
          <ul>
            ...
            <li class="nav_active"><i class="nav_6"></i><router-link to="/csgn">城市供暖</router-link></li>
        ...
        <div class="div_any">
            <div class="left div_left_any10">
                <div class="div_any_child">
                    <div class="div_any_title"><img src="../../static/images/title_1.png">城市供暖情况统计</div>
                    <div id="Chart01" class="p_chart" style="width:97.5%;height:95%;display: inline-block;padding-left: 1.25%;padding-top:2.2%"></div>
                </div>
            </div>
            <div class="left div_right_any">
                <div class="div_any_child ">
                    <div class="div_any_title"><img src="../../static/images/title_1.png">城市供暖情况统计</div>
                    <div id="Chart02" class="p_chart" style="width:97.5%;height:95%;display: inline-block;"></div>
                </div>
                <div class="div_any_child">
                    <div class="div_any_title"><img src="../../static/images/title_1.png">被投诉单位排名统计</div>
                    <div id="Chart04" class="p_chart" style="width:97.5%;height:95%;display: inline-block;padding-left: 1.25%;padding-top:2.2%"></div>
                </div>
            </div>
        </div>
    </div>
...
<script>
import {drawPie02} from "../utils/utils.js"
import {drawBar_01} from "../utils/utils.js"
import {drawHistogramChart3} from "../utils/utils.js"
...
export default {
  ...
  mounted(){
    ...
    //建设 - 物业管理 三级定性排行榜前三
    this.get(host.url+'/gnphb').then(res=>{
      res.data[0].legend = this.fzarr(res.data[0].legend);
      res.data[0].data =this.fzarr(res.data[0].data);
      //从组数组便于绘制图标
      this.HistogramChart2Series = {name:'城市供暖',type:'bar',data:res.data[0].data,itemStyle: {
            normal: {
                label: {
                    show: true,		//开启显示
      ...
      //画饼状图
      this.drawPie02('Chart02',res.data[0].legend,'城市供暖',newdata);
    })
    //三级分类排行榜前5
    this.get(host.url+'/gntsphb').then(res=>{
      this.btsLegend = res.data[0].legend
      this.btsData = res.data[0].data
      //画柱形图
      this.drawBar_01('Chart04',this.btsLegend,'城市供暖',this.btsData);
    })
  ...
  methods:{
    ...
    search:function(){
      ...
      //三级分类排行榜前5
      this.get(host.url+'/sjphb',{start:this.start,end:this.end,id:this.id}).then(res=>{
        this.btsLegend = res.data[0].legend
        this.btsData = res.data[0].data
        //画柱形图
        this.drawBar_01('Chart04',this.btsLegend,'城市供暖',this.btsData);
